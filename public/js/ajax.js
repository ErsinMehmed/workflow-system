$(document).ready(function () {
  ...
  function filterScheduleUsers(text, position) {
    postData(
      "action/user/FilterSchedule.php",
  ...
  function filterUsers(text, position, status, pagination, record) {
    postData(
  ...
  $(document).on("submit", "#generate-password-form", function (e) {
    e.preventDefault();

    const formData = new FormData(this);
    formData.append("change_customer_password", true);

    $("#customer-forgot-password-spinner").removeClass("hidden");
    $("#generate-password-form").addClass("animate-pulse opacity-20");

    postFormData("action/Customer.php", formData, function (response) {
      const res = JSON.parse(response);

      if (res.status == 200) {
        $("#customer-forgot-password-spinner").addClass("hidden");
        $("#generate-password-form").removeClass("animate-pulse opacity-20");
        closeModal("#customer-password-modal");
        alertify.success(res.message);
        $("#generate-password-form")[0].reset();
      } else if (res.status == 500) {
        alertify.error(res.message);
        $("#customer-forgot-password-spinner").addClass("hidden");
        $("#generate-password-form").removeClass("animate-pulse opacity-20");
  ...
  $(".phone-code").keyup(function () {
    const email = $("#get-customer-email").val();
    const inputs = $(
      "#first-num-p, #second-num-p, #third-num-p, #fourth-num-p, #fifth-num-p, #sixth-num-p"
    );
    const code = inputs
      .map((i, el) => $(el).val())
      .get()
    ...
    if (code.length > 5) {
      getData(`action/Customer.php?email=${email}`, function (response) {
        ...
        if (res.status == 200) {
          if (res.data.phone_code == code) {
            const verifyPhone = $("#get-customer-email").val();
            ...
            postData(
              ...
              function (response) {
                ...
                if (res.status == 200) {
                  ...
                  $("#alert-additional-content-mobile").addClass("hidden");
                  alertify.success(res.message);
                }
              }
            );
  ...
  $(document).on("click", ".history-view", function () {
    ...
    getData(`action/Customer.php?id=${id}`, function (response) {
      ...
      if (res.status == 200) {
        ...
        const created = `${day}.${month}.${year}г.`;
    ...
    getData(`action/AdminUsers.php?id=${id}`, function (response) {
      ...
      if (res.status == 200) {
        ...
        $("#user-position-edit")
          ...
          .find(`option[value="${res.data.position}"]`)
          .attr("selected", "selected");
        $("#user-hourly-rate-edit")
          .val(res.data.hourly_rate)
          .find(`option[value="${res.data.hourly_rate}"]`)
          .attr("selected", "selected");
  ...
  const debouncedUsersFilter = debounce(function () {
    filterUsers(
      ...
      $("#select-position").val(),
      $("#select-status").val(),
      $("#user-table-pagination").val(),
      $("#select-records-count-user").val()
    );
  });

  $("#search-user").on("keyup", debouncedUsersFilter);

  // Change user table page
  $(document).on("click", ".user-pagination", function () {
    $("#user-table-pagination").val($(this).val());
  ...
  // User search by name or pid schedule
  const debouncedUsersScheduleFilter = debounce(function () {
    filterScheduleUsers(
      $("#search-user-schedule").val(),
      $("#select-position-user").val()
    );
  });

  $("#search-user-schedule").on("keyup", debouncedUsersScheduleFilter);
  ...
  // User record count
  $(document).on("change", "#select-records-count-user", function () {
    filterUsers(
      $("#search-user").val(),
      $("#select-position").val(),
      $("#select-status").val(),
  ...
  // User position filter schedule
  $("#select-position-user").on("change", function () {
    filterScheduleUsers($("#search-user-schedule").val(), $(this).val());
  });
  ...
  // Owner change target
  $(document).on("submit", "#change-target-value", function (e) {
    e.preventDefault();

    const formData = new FormData(this);
    formData.append("owner_change_target", true);
  ...
  // User table column sort
  $(document).on("click", ".user_column_sort", function () {
    ...
    const pagination = $("#user-table-pagination").val();
    const record = $("#select-records-count-user").val();
    const order = $(this).data("order");
    console.log(order);

    var arrow = "";
    if (order == "desc") {
      arrow = `<svg class="w-3.5 h-3.5 text-red-700" stroke="currentColor" viewBox="0 0 1900 1900">
  ...
  // Owner reject the question
  $(document).on("click", "#owner-question-rejected", function () {
    ...
    postData(
      ...
      function (response) {
        ...
        if (res.status == 200) {
          $("#owner-question-section").load(
            location.href + " #owner-question-section"
          );
          alertify.success(res.message);
        }
      }
  ...
  // Owner accept the question
  $(document).on("click", "#owner-question-accepted", function () {
    const questionId = $(this).val();
    const state = 1;
    ...
    getData(`action/Customer.php?email=${email}`, function (response) {
      ...
      if (res.status != 404) {
        const imageProperties = [
          { id: "first_img", data: "image_room1" },
          { id: "second_img", data: "image_room2" },
          { id: "third_img", data: "image_room3" },
        ];
  ...
  // Mobile user ask question
  $(document).on("submit", "#user-question-form", function (e) {
    e.preventDefault();

    const formData = new FormData(this);
    formData.append("mobile_ask_question", true);
    ...
    postFormData("action/Mobile.php", formData, function (response) {
      ...
      if (res.status == 200) {
        alertify.success(res.message);
        $("#user-question-form")[0].reset();
        closeModal("#user-question-modal");
      } else if (res.status == 500) {
        alertify.error(res.message);
      }
    });
