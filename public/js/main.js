...
$(document).ready(function () {
  ...
  closeModal("#close-phone-verify-modal", "#phone-verify-modal");

  openModal("#phone-verify-btn", "#phone-verify-modal");

  openModal("#add-admin-btn", "#add-admin-modal");

  closeModal(".close-add-admin-modal", "#add-admin-modal");

  openModal("#show-user-schedule", "#user-schedule-modal");

  closeModal(".close-user-schedule-modal", "#user-schedule-modal");

  openModal(".open-map-modal", "#map-modal");

  closeModal(".close-map-modal", "#map-modal");

  closeModal(".close-edit-admin-modal", "#edit-admin-modal");

  openModal("#add-user-question", "#user-question-modal");

  openModal("#change-password-btn", "#customer-password-modal");

  closeModal(".customer-close-password-modal", "#customer-password-modal");

  closeModal(".close-user-question-modal", "#user-question-modal");

  openModal("#show-user-question", "#user-my-question-modal");

  closeModal(".close-user-my-question-modal", "#user-my-question-modal");
  ...
  // Go to next or prev input phone verification
  const phoneInputs = document.querySelectorAll(".phone-code");

  phoneInputs.forEach((input, index) => {
    input.addEventListener("input", function () {
      if (input.value.length === 1) {
        if (!phoneInputs[index + 1].value) phoneInputs[index + 1].focus();
      } else if (input.value.length === 0 && index > 0) {
