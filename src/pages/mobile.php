...
<body>
          ...
          <div class="px-3 py-4 overflow-y-auto bg-[#f8f8f8] h-screen">
            <ul class="flex-col space-y-4 text-slate-600">
              <li id="profile-btn" @click="mobProfile = true; mobOrder = false; mobWarehouse = false">
                <div class="flex items-center justify-center w-16 h-16 mx-auto rounded-full border-4 border-white bg-[#cde8f8] cursor-pointer mt-2 transition-all active:scale-90 shadow-xl ring-1 ring-slate-200"><img class="object-cover h-12 w-12" src="images/title.png" alt="Company icon" /></div>
            ...
            <li id="mobile-log-out" class="h-[88px] w-[88px] bg-white rounded-md shadow-xl border border-slate-100 flex items-center justify-center cursor-pointer transition-all active:scale-90 text-slate-600 mt-[246px]">
              <div>
                <svg fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 mx-auto">
          ...
          <div class="ml-28">
            <div v-show="mobProfile">
              ...
              <div class="flex p-4 space-x-5">
                <div class="w-[40%] shadow-xl border border-slate-100 flex items-center justify-center py-[8px] text-slate-600 rounded-md">
                  <?php
                  $query = "SELECT id, name, position, pid FROM users WHERE pid = '$pid'";
                  $query_run = mysqli_query($con, $query);

                  while ($rows = mysqli_fetch_array($query_run)) { ?>
                    <div>
                      <svg viewBox="0 0 24 24" fill="currentColor" class="w-36 h-36 mt-6 mx-auto text-[#407cb2]">
                        <path fill-rule="evenodd" d="M7.5 6a4.5 4.5 0 119 0 4.5 4.5 0 01-9 0zM3.751 20.105a8.25 8.25 0 0116.498 0 .75.75 0 01-.437.695A18.683 18.683 0 0112 22.5c-2.786 0-5.433-.608-7.812-1.7a.75.75 0 01-.437-.695z" clip-rule="evenodd" />
                      </svg>
                      <div class="text-2xl uppercase font-bold mt-2 text-center"><?= $rows['name'] ?></div>
                      <div class="text-xl text-center font-semibold"><?= $rows['position'] ?></div>
                      <div class="text-xl text-center font-semibold"><?= $rows['pid'] ?></div>
                      <div class="text-lg mt-20 text-center">IP ADDRESS: 10.192.217.58</div>
                      ...
                      <div class="text-center text-sm font-bold mb-20">ver 1.14 &copy; 26.01.2023</div>
                      <button id="show-user-schedule" value="<?= $rows['id'] ?>" type="button" class="w-full px-2.5 py-1.5 text-sm font-semibold uppercase bg-white border border-[#407cb2] text-[#407cb2] focus:outline-none transition-all active:scale-90 rounded">График</button>
                      <button id="show-user-question" type="button" class="w-full my-3.5 px-2.5 py-1.5 text-sm font-semibold uppercase bg-white border border-[#407cb2] text-[#407cb2] focus:outline-none transition-all active:scale-90 rounded">Моите въпроси</button>
                      <button id="add-user-question" type="button" class="w-full px-2.5 py-1.5 text-sm font-semibold uppercase bg-white border border-[#407cb2] text-[#407cb2] focus:outline-none transition-all active:scale-90 rounded">Задай въпрос</button>
                    </div>
                  <?php } ?>
                </div>
              ...
              <div class="p-4">
                <div class="hidden bg-white absolute left-28 top-14 inset-0 z-40 order-start-loader">
                  <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
                    <div role="status">
                      <svg aria-hidden="true" class="w-12 h-12 text-gray-200 animate-spin fill-blue-600 mx-auto" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor" />
                        <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentFill" />
                      </svg>
  ...
  <div id="order-photo-modal" class="bg-gray-800 hidden bg-opacity-50 fixed inset-0 z-40">
    <div class="h-full w-full p-5 overflow-x-hidden overflow-y-auto flex justify-center items-center">
      <div class="relative w-full h-auto animate__animated animate__zoomIn animate__fast max-w-2xl">
        <div class="relative bg-white rounded shadow mb-6">
          <div class="text-xl font-bold text-white bg-blue-500 py-2 rounded-t text-center">
            Снимки на обекта
          </div>
          <div class="p-4 pb-1 text-center flex items-center justify-center space-x-4">
            <img id="first_img" class="w-[202px] h-[202px] object-cover rounded hidden">
            <img id="second_img" class="w-[202px] h-[202px] object-cover rounded hidden">
            <img id="third_img" class="w-[202px] h-[202px] object-cover rounded hidden">
          </div>
  ...
  <div id="user-question-modal" class="bg-gray-800 hidden bg-opacity-50 fixed inset-0 z-40">
    <div class="h-full w-full p-5 overflow-x-hidden overflow-y-auto flex justify-center items-center">
      <div class="relative w-full h-auto max-w-md animate__animated animate__zoomIn animate__fast">
        <div class="relative bg-white rounded shadow mb-6">
          <div class="text-xl font-bold text-white bg-blue-500 py-2 rounded-t text-center">
          ...
          <form id="user-question-form">
            <div class="px-4 text-center">
              ...
              <textarea rows="4" name="text" class="block p-2.5 w-full text-sm text-slate-700 rounded border border-slate-300 focus:outline-none mt-4 resize-none" placeholder="Напишете въпроса си тук..."></textarea>
            </div>
            <div class="flex items-center p-4">
              <button type="button" class="flex-1 px-4 py-1.5 bg-red-500 text-white text-sm font-semibold rounded active:scale-90 transition-all close-user-question-modal">
                Затвори
              </button>
              <button type="submit" class="flex-1 px-4 py-1.5 bg-blue-500 text-white text-sm font-semibold rounded active:scale-90 transition-all ml-3">
                Запази
  ...
  <div id="user-my-question-modal" class="bg-gray-800 hidden bg-opacity-50 fixed inset-0 z-40">
    <div class="h-full w-full p-5 overflow-x-hidden overflow-y-auto flex justify-center items-center">
      <div class="relative w-full h-auto max-w-xl animate__animated animate__zoomIn animate__fast">
        <div class="relative bg-white rounded shadow mb-6">
          <div class="text-xl font-bold text-white bg-blue-500 py-2 rounded-t text-center">
            Моите въпроси
          </div>
          <div class="px-4 text-center space-y-4 pt-4">
            <?php
            $query = "SELECT * FROM user_questions WHERE user_pid = '$pid' LIMIT 6";
            $query_run = mysqli_query($con, $query);

            if (mysqli_num_rows($query_run) > 0) {
              while ($rows = mysqli_fetch_array($query_run)) { ?>
                <div class="w-full flex items-cneter shadow-lg rounded-md border border-slate-50">
                  <div class="w-2/5 border-x rounded-l-md pl-2">
                    <div class="text-center py-1 text-slate-700 font-semibold text-sm">Въпрос</div>
                    <div class="text-center pb-1 text-slate-700 text-sm"><?= $rows["question"] ?></div>
                  </div>
                  <div class="w-1/5 border-r px-2">
                    <div class="text-center py-1 text-slate-700 font-semibold text-sm">Админ</div>
                    <?php if ($rows['admin_approval'] == NULL) { ?>
                      <div class="w-8 h-8 rounded-full flex items-center justify-center mx-auto bg-gray-100 text-gray-400 shadow-md font-semibold text-lg border border-gray-200">?</div>
                    <?php } else if ($rows['admin_approval'] == 0) { ?>
                      <div class="w-8 h-8 rounded-full flex items-center justify-center mx-auto bg-red-100 text-red-400 shadow-md font-semibold text-lg border border-red-200">
                        <svg fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-5 h-5">
                          <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
                        </svg>
                      </div>
                    <?php } else if ($rows['admin_approval'] == 1) { ?>
                      <div class="w-8 h-8 rounded-full flex items-center justify-center mx-auto bg-green-100 text-green-400 shadow-md font-semibold text-lg border border-green-200">
                        <svg fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-5 h-5">
                          <path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                        </svg>
                      </div>
                  ...
                  <div class="w-1/5 px-2">
                    <div class="text-center py-1 text-slate-700 font-semibold text-sm">Собственик</div>
                    <?php if ($rows['owner_approval'] == NULL && $rows['admin_approval'] != 0 || $rows['admin_approval'] == NULL) { ?>
                      <div class="w-8 h-8 rounded-full flex items-center justify-center mx-auto bg-gray-100 text-gray-400 shadow-md font-semibold text-lg border border-gray-200">?</div>
                    <?php } else if ($rows['owner_approval'] == 0) { ?>
                      <div class="w-8 h-8 rounded-full flex items-center justify-center mx-auto bg-red-100 text-red-400 shadow-md font-semibold text-lg border border-red-200">
                        <svg fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-5 h-5">
                          <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
                        </svg>
                      </div>
                    <?php } else if ($rows['owner_approval'] == 1) { ?>
                      <div class="w-8 h-8 rounded-full flex items-center justify-center mx-auto bg-green-100 text-green-400 shadow-md font-semibold text-lg border border-green-200">
                        <svg fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-5 h-5">
                          <path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                        </svg>
                  ...
                  <div class="w-1/5 border-x rounded-r-md text-slate-700 text-sm pr-2">
                    <div class="text-center py-1 font-semibold">Дата</div>
                    <div class="text-center pb-1 text-slate-700 text-sm"><?= date("d.m.Y H:i", strtotime($rows['add_date'])) ?></div>
                  </div>
                </div>
              <?php }
            } else { ?>
              <div class="text-slate-700 font-semibold text-center py-8">Нямате зададени въпроси</div>
            <?php } ?>
          </div>
          <div class="p-4">
            <button type="button" class="w-full px-4 py-1.5 bg-slate-100 border border-slate-200 text-slate-700 text-sm font-semibold rounded active:scale-90 transition-all close-user-my-question-modal">
              Затвори
            </button>
          </div>
        </div>
  ...
  <div id="user-schedule-modal" class="bg-gray-800 hidden bg-opacity-50 fixed inset-0 z-40">
    <div class="h-full w-full p-5 overflow-x-hidden overflow-y-auto flex justify-center items-center">
      <div class="relative w-full h-auto max-w-lg animate__animated animate__zoomIn animate__fast">
        <div class="relative bg-white rounded shadow mb-6">
          ...
          <div class="flex items-center justify-start pt-4 px-6">
          ...
          <div class="p-4">
            <button type="button" class="w-full px-4 py-1.5 bg-slate-100 border border-slate-200 text-slate-700 text-sm font-semibold rounded active:scale-90 transition-all close-user-schedule-modal">
              Затвори
            </button>
          </div>
        </div>
        ...
        <form id="mobile-login-form" class="mt-6 mb-0 space-y-4 rounded-lg p-8 shadow-2xl text-slate-700">
          ...
          <div>
            ...
            <div class="relative mt-1">
              <input id="login-pid" type="text" name="pid" class="w-full rounded-lg border border-slate-50 p-2.5 pr-12 text-sm shadow-sm focus:border-slate-100 focus:outline-none" placeholder="Въведи ПИД" />
              <span class="absolute inset-y-0 right-4 inline-flex items-center">
                <svg fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="h-5 w-5 text-gray-400">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" />
                </svg>
              </span>
            </div>
          ...
          <div>
            ...
            <div class="relative mt-1">
              ...
              <span @click="mobileLoginPass = !mobileLoginPass" class="absolute inset-y-0 right-4 inline-flex items-center transition-all active:scale-90">
                <svg v-show="!mobileLoginPass" class="h-5 w-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                </svg>
                <svg v-show="mobileLoginPass" class="h-5 w-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor">
